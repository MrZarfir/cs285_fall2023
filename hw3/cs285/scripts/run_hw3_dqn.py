...
def run_training_loop(config: dict, logger: Logger, args: argparse.Namespace):
    ...
    for step in tqdm.trange(config["total_steps"], dynamic_ncols=True):
        ...
        # DONE TODO(student): Compute action
        action = agent.get_action(observation, epsilon)

        # DONE TODO(student): Step the environment
        next_observation, reward, done, info = env.step(action)

        next_observation = np.asarray(next_observation)
        truncated = info.get("TimeLimit.truncated", False)
        episode_ended = done and (not truncated)
        # DONE TODO(student): Add the data to the replay buffer
        if isinstance(replay_buffer, MemoryEfficientReplayBuffer):
            # We're using the memory-efficient replay buffer,
            # so we only insert next_observation (not observation)
            replay_buffer.insert(np.array(action), reward, next_observation, episode_ended)
        else:
            # We're using the regular replay buffer
            replay_buffer.insert(observation, np.array(action), reward, next_observation, episode_ended)
        ...
        if step >= config["learning_starts"]:
            ...
            dones = batch["dones"]
            update_info = agent.update(obs, acs, rewards, next_obs, dones, step)
